...
func init() {
	...
	fetchCommand.FlagSet.StringVar(&f.FindPackageOption, "find-package-arg", "NO_DEFAULT_PATH", "global options for find_package when generating file pkg.dep.cmake")
...
type fetch struct {
	...
	FindPackageOption string // global find_package option, default is "NO_DEFAULT_PATH".
...
{{end}}`); err != nil {
	...
	if err := createPkgDepCmake(f.PkgHome, &f.DepTree, f.FindPackageOption); err != nil {
		return err
	}
