...
func init() {
	...
	getCommand.FlagSet.StringVar(&pkgHome, "p", pwd, "home path for file pkg.json")
...
func (get *get) Run() error {
	...
	if err := buildPkg(&get.DepTree, get.PkgHome, &pkgBuiltSet); err != nil {
		return err
	}

	// generating cmake script to include this lib.
	// the generated cmake file is stored at where pkg command runs.
	if cmakeDepWriter, err := os.Create(filepath.Join(get.PkgHome, utils.CMakeDep)); err != nil {
		return err
	} else {
		...
		if err := cmakeLib(&get.DepTree, get.PkgHome, &pkgCMakeLibSet, bufWriter); err != nil {
			return err
		}
		bufWriter.Flush()
	}
...
func (get *get) installSubDependency(installPath string, depTree *DependencyTree) error {
	if pkgJsonPath, err := os.Open(filepath.Join(installPath, utils.PkgFileName)); err == nil { // pkg.json exists.
		...
		if bytes, err := ioutil.ReadAll(pkgJsonPath); err != nil { // read file contents
			...
			if err := get.dlSrc(get.PkgHome, &pkgs.Packages, &get.DepTree); err != nil {
				return err
			}
			...
			if build, ok := pkgs.Build[runtime.GOOS]; ok {
				get.DepTree.SelfBuild = build[:]
			}
			// add cmake include script for this lib
			get.DepTree.SelfCMakeLib = pkgs.CMakeLib
			return nil
		}
	} else {
